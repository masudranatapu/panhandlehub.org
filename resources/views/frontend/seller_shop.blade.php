    ...
    <div class="container">
        <div class="seller_dashboard mb-4 mt-5">
            <div class="d-flex  position-relative align-items-center">
                ...
                <div class="info_seller">
                    ...
                    <div class="star">
                        @for ($i = 0; $i < 5; $i++) @if ($i < $reviews->avg('stars'))
        ...
        <div class="seller_dashboard_shop">
            <ul class="nav nav-pills mb-5" id="pills-tab" role="tablist" style="justify-content: center">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="write_review_id" data-bs-toggle="pill"
                        data-bs-target="#write_review_tab" type="button" role="tab" aria-controls="write_review_tab"
                        aria-selected="false">Write Review
                    </button>
                </li>
                @endif
            </ul>
            <div class="tab-content" id="pills-tabContent">
                                        ...
                                        <form action="{{ route('seller.profile', $seller->username) }}" method="get"
                                            ...
                                            <div class="input-group">
                                                ...
                                                <select name="sort" id="sort" class="form-control"
                                                    ...
                                                    <option value="high_to_low" {{ request()->sort == 'high_to_low' ?
                                                        'selected' : '' }}>
                                                        Price high to low
                                                    </option>
                                                    <option value="low_to_high" {{ request()->sort == 'low_to_high' ?
                                                        'selected' : '' }}>
                                                        Price low to high
                                                    </option>
                                                </select>
                                ...
                                <div class="feedback_list">
                                    <div class="d-md-flex position-relative">
                                        ...
                                        <div class="feedback_content">
                                            <h3>
                                                <a href="{{ route('seller.profile', $review->user->username) }}">{{
                                                    $review->user->name ?? $review->user->username }}</a>
                                            </h3>

                                            <h6>{{ date('F d, Y', strtotime($review->created_at)) }}</h6>
                                            <div class="star">
                                                @for ($i = 0; $i < 5; $i++) @if ($i < $review->stars)
                                                    <img src="{{ asset('frontend/images/icon/star.svg') }}" alt="star">
                                                    @else
                ...
                @if ((Auth::check() && $seller->id != Auth::user()->id) || !Auth::check())
                <!-- write review -->
                <div class="tab-pane fade" id="write_review_tab" role="tabpanel" aria-labelledby="write_review_id"
                    tabindex="0">
